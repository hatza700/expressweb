...
' This file implements the code-behind class for AddTTrafficSafety_26_QuangCao.aspx.
' App_Code\AddTTrafficSafety_26_QuangCao.Controls.vb contains the Table, Row and Record control classes
...
Partial Public Class AddTTrafficSafety_26_QuangCao
        Inherits BaseApplicationPage
' Code-behind class for the AddTTrafficSafety_26_QuangCao page.
...
#Region "Section 1: Place your customizations here."
      Protected Sub btnDoPost_back_noinhan_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDoPost_back_noinhan.Click
            ...
            '  Me.Page.Response.Write(Me.hdh_image_noinhan.Value.ToString + "-1asadas")
           ' Dim strxxx As String = ""
           ' strxxx = "aaaaa"
           ' ScriptManager.RegisterClientScriptBlock(Me, [GetType](), "none11", "<script>fuck_off();</script>", False)
            ' Me.lit_image_demonstrade.Text = "<div style='display:inline-block;width:150px;height:150px;padding:3px;border-radius:3px;border:1px solid #ccc;padding-bottom:5px;padding-top:5px;'><img src='" + CMD.CMS_APP_NAME.ToString.Trim + "Customize_style/icons/no_images.png' style='width:140px;border:0;' />" + "</div>"
            Dim str_duongdan_anh As String

            str_duongdan_anh = Me.hdh_image_noinhan.Value.ToString

            str_duongdan_anh = str_duongdan_anh.Replace("../", "")

            If str_duongdan_anh.Length > 0 Then
                Dim CMD As New CMS_SHARED
                Me.lit_image_demonstrade.Text = "<div style='display:inline-block;width:150px;height:150px;padding:3px;border-radius:3px;border:1px solid #ccc;padding-bottom:5px;padding-top:5px;'><img src='" + _
                CMD.CMS_APP_NAME.ToString.Trim + str_duongdan_anh + "' style='width:140px;border:0;' />" + "</div>"
                Me.DuongDanAnh.Text = str_duongdan_anh
            End If
        End Sub
      ...
      Public Sub LoadData()
          ...
          LoadData_Base()
            write_log("Truy cập trang:Thêm quảng cáo")

            Me.ViTri.AutoPostBack = True
            Me.ViTri.Attributes.Add("onclick", "javascript:__doPostBack('','')")
...
'Public Sub PushButton1_Click(ByVal sender As Object, ByVal args As EventArgs)
'          ' Click handler for PushButton1.
'          ' Customize by adding code before the call or replace the call to the Base function with your own code.
'          PushButton1_Click_Base(sender, args)
'          ' NOTE: If the Base function redirects to another page, any code here will not be executed.
'        End Sub
		Public Sub PushButton_Click(ByVal sender As Object, ByVal args As EventArgs)
            ' Click handler for PushButton.
            ' Customize by adding code before the call or replace the call to the Base function with your own code.
            Dim strimg As String = ""
            strimg = hdh_image_noinhan.Value.ToString
            
			 Dim _CMD As New CMS_SHARED
            Dim app_root_folder As String
            app_root_folder = _CMD.CMS_APP_NAME
            Dim strx As String = ""
            Dim _image As String

            If strimg.Length > 0 Then
                _image =strimg
                ' /CQ_tTrafficSafety/_SHARED_FILES/alo/blue-eyed-girl-with-freckles-girl-hd-wallpaper-1920x1080-2433.jpg
                _image = _image.Replace("/CQ_tTrafficSafety/", "")
                _image = _image.Replace("/dctQuanTri/", "")
				_image = _image.Replace("http://localhost", "")
				'http://localhost
                strx = "<img src='" + app_root_folder + _image + "' style=""width:140px;border:0;"">"
                ' Page.response.write(_image)
                Me.lit_image_demonstrade.Text = "<div style=""display:inline-block;width:150px;height:150px;padding:3px;border-radius:3px;border:1px solid #ccc;padding-bottom:5px;padding-top:5px;"">" + strx + "</div>"
                Me.DuongDanAnh.Text = _image
            Else
                ' Me.Page.Response.Write("fuck off")
            End If
            PushButton_Click_Base(sender, args)
            ' NOTE: If the Base function redirects to another page, any code here will not be executed.
        End Sub
#End Region
...
#Region "Section 2: Do not modify this section."
        ...
        Protected Sub Page_InitializeEventHandlers_Base(ByVal sender As Object, ByVal e As System.EventArgs)            		
          ...
          ' Setup the pagination events.
            ...
            AddHandler Me.PushButton.Click, AddressOf PushButton_Click
            ...
            Me.SaveAndNewButton.Button.Attributes.Add("onclick", "SubmitHRefOnce(this, """ & Me.GetResourceValue("Txt:SaveRecord", "CMStTrafficSafety") & """);")
        ...
        Private Sub Base_RegisterPostback()
              ...
              Me.RegisterPostBackTrigger(MiscUtils.FindControlRecursively(Me,"PushButton"))
    ...
    End Function
      ...
      Public Sub SaveData_Base()
        ...
        Me.TTrafficSafety_26_QuangCaoRecordControl.SaveData()
      ...
      End Sub  
        ...
        Me.TTrafficSafety_26_QuangCaoRecordControl.LoadData()
        Me.TTrafficSafety_26_QuangCaoRecordControl.DataBind()
                ...
                If Not pageMaster is Nothing Then
                    ...
                    If Not menuComponent is Nothing Then
                        menuComponent.HiliteSettings = "TTrafficSafety_26_QuangCaoMenuItem"
                    End If
                End If
        ...
        Public Sub PushButton_Click_Base(ByVal sender As Object, ByVal args As EventArgs)
    ...
    Try
        ...
        Public Sub CancelButton_Click_Base(ByVal sender As Object, ByVal args As EventArgs)
            ...
            ' redirected to the URL.
                  ...
                  Dim url As String = "../tTrafficSafety_26_QuangCao/ShowTTrafficSafety_26_QuangCaoTable.aspx"
...
#End Region
        ...
        Protected Sub ViTri_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ViTri.SelectedIndexChanged
            Dim _strX As String = ""
            _strX = Me.ViTri.SelectedValue.ToString
            '  Me.Page.Response.Write(_strX)
