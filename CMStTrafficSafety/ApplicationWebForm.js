...
function setSearchBoxText(name) {
    var searchControl = document.getElementById(name);
    if (searchControl != null) {
        if (searchControl.value == '' || searchControl.value == 'Search for...') {
            searchControl.value = 'Search for...';
...
function handleEnterInText(field) {
    var i;
    for (i = 0; i < field.form.elements.length; i++)
        if (field == field.form.elements[i])
        break;

    // Set focus on the next focusable element (text, password, or textarea)
    while (++i < field.form.elements.length) {
        var elem = field.form.elements[i];
        if (Fev_IsFocusableElement(elem)) {
            elem.focus();
            break;
        }
    }
