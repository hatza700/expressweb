...
Partial Class SHARED_FILES_Select_Image
...
#Region "PAGE METHODS"
        ...
        If straction IsNot Nothing = True Then
            ...
            Select Case strxxx
                ...
                Case "edit_image"
                    ...
                    Exit Select
                Case "selected_image"
                    ' Me.Page.Response.Write(hdh_action.Value.ToString)
                    func_selectimage()
                    Exit Select
                Case "other"
                    func_images_selected()
    ...
    Sub func_selectimage()

        Dim file_name As FileInfo
        Dim file_path As String
        Dim str_image_full_name As String = image_full_name.Value.ToString

        file_path = System.Web.HttpContext.Current.Server.MapPath(str_image_full_name)
        file_name = New FileInfo(file_path)
        Dim str_html As String = ""
        Dim file_names As String = hdh_curent_image.Value.ToString
        _cms = New CMS_SHARED
        If file_name.Exists Then
            str_html += "<div style='height:20px;text-align:left;vertical-align:middle;margin-left:5px;'>Bạn đã chọn ảnh <input type=button value='Chọn ảnh khác' onclick='javascript:chonanhkhac()' /></div>" + vbCrLf
            ...
            If file_names.Equals("web.config", StringComparison.CurrentCultureIgnoreCase) = False Then
                str_image_full_name = str_image_full_name.Replace("../", "")
                str_html += "<li id='li" + "0".ToString + "' style='width:320px;height:230px;' class='normal' >"
                str_html += "<a class='ui-btn'>"
                str_html += "<img src='" + _cms.CMS_APP_NAME + str_image_full_name + "' style='width:280px;height:180px; padding-top:10px;' class='ui-li-thumb' />"
                str_html += "</a>"
    ...
    Private Sub func_images_selected()
        ...
        If str_selected_folder.Length > 0 Then
            ...
            While i < count
                ...
                str_html += "<li id='li" + i.ToString + "' style='width:90px;height:130px;' class='normal' onclick='javascript:change_class(""li" + i.ToString + """,""normal"",""selectedli"",""" + folder_name + """,""" + folder_path + """)'>"
                str_html += "<a class='ui-btn'>"
                str_html += "<img src='" + _cms.CMS_APP_NAME + "DashBoard/Scripts/images/1445346519_icon-96-folders.png" + "' style='width:80px;padding-top:5px;' class='ui-li-thumb' />"
                str_html += "</a>"
            ...
            If jlen > 0 Then
                While j < jlen
                    ...
                    'Me.Page.Response.Write(folder_path.ToString + "</br>")
                    If file_names.Equals("web.config", StringComparison.CurrentCultureIgnoreCase) = False Then
                        str_html += "<li id='li" + j.ToString + "' style='width:90px;height:130px;' class='normal' onclick='javascript:change_class_images(""li" + j.ToString + """,""normal"",""selectedli"",""" + file_names + """,""" + file_paths + """)'>"
                        str_html += "<a class='ui-btn'>"
                        str_html += "<img src='" + _cms.CMS_APP_NAME + root_folder + file_names + "' style='width:80px; padding-top:10px;' class='ui-li-thumb' />"
                        str_html += "</a>"
                        str_html += "<div class=""ckf-file-desc ui-bar-a"" draggable=""true""><h2 title=""" + file_names + """>" + file_names + "</h2></div>"
                        str_html += "</li>"
                    End If
