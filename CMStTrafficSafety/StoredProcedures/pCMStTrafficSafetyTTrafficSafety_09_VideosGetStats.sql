if exists (select * from sysobjects where id = object_id(N'pCMStTrafficSafetyTTrafficSafety_09_VideosGetStats') and sysstat & 0xf = 4) drop procedure pCMStTrafficSafetyTTrafficSafety_09_VideosGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pCMStTrafficSafetyTTrafficSafety_09_VideosGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''tTrafficSafety_09_Videos'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[tTrafficSafety_09_Videos] TTrafficSafety_09_Videos_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
