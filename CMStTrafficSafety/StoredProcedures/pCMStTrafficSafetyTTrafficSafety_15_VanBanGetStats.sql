if exists (select * from sysobjects where id = object_id(N'pCMStTrafficSafetyTTrafficSafety_15_VanBanGetStats') and sysstat & 0xf = 4) drop procedure pCMStTrafficSafetyTTrafficSafety_15_VanBanGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pCMStTrafficSafetyTTrafficSafety_15_VanBanGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''tTrafficSafety_15_VanBan'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[tTrafficSafety_15_VanBan] TTrafficSafety_15_VanBan_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
