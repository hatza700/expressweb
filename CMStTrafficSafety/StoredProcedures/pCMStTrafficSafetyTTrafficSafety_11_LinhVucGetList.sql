if exists (select * from sysobjects where id = object_id(N'pCMStTrafficSafetyTTrafficSafety_11_LinhVucGetList') and sysstat & 0xf = 4) drop procedure pCMStTrafficSafetyTTrafficSafety_11_LinhVucGetList 
GO
...
-- Returns a query resultset from table [dbo].[tTrafficSafety_11_LinhVuc]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pCMStTrafficSafetyTTrafficSafety_11_LinhVucGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[tTrafficSafety_11_LinhVuc] TTrafficSafety_11_LinhVuc_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY TTrafficSafety_11_LinhVuc_.[MaLinhVuc] asc '
        ...
        SET @l_temp_colsWithAlias = 
            N'TTrafficSafety_11_LinhVuc_.[MaLinhVuc]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'TTrafficSafety_11_LinhVuc_.[MaLinhVuc],
            TTrafficSafety_11_LinhVuc_.[TenLinhVuc],
            TTrafficSafety_11_LinhVuc_.[GhiChu],
            TTrafficSafety_11_LinhVuc_.[OrderID],
            CAST(BINARY_CHECKSUM(TTrafficSafety_11_LinhVuc_.[MaLinhVuc],TTrafficSafety_11_LinhVuc_.[TenLinhVuc],TTrafficSafety_11_LinhVuc_.[GhiChu],TTrafficSafety_11_LinhVuc_.[OrderID]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [MaLinhVuc] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON TTrafficSafety_11_LinhVuc_.[MaLinhVuc] = IS_ALIAS.[MaLinhVuc] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(TTrafficSafety_11_LinhVuc_.[MaLinhVuc],TTrafficSafety_11_LinhVuc_.[TenLinhVuc],TTrafficSafety_11_LinhVuc_.[GhiChu],TTrafficSafety_11_LinhVuc_.[OrderID]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[tTrafficSafety_11_LinhVuc] TTrafficSafety_11_LinhVuc_ ' + 
            'WHERE 1=2;'
