...
#Region "Section 1: Place your customizations here."
        ...
        Public Sub LoadData()
			...
			'System.Web.HttpContext.Current.Response.Redirect("aaa")
	...
	End Sub
        ...
        Private Sub login_custoize(ByVal bRedirectOnSuccess As Boolean)
            '====================================================================


            Dim strSql As String = "proc_admins_check_account"

            Dim _CMS As New CMS_SHARED

            Dim CMS_CONNECTIONSTRING As String = _CMS.CMS_CONNECTIONSTRING

            Dim strConnString As String = ConfigurationManager.ConnectionStrings(CMS_CONNECTIONSTRING).ConnectionString
            Using con As New SqlConnection(strConnString)
                Using cmd As New SqlCommand()
                    cmd.CommandText = strSql
                    cmd.CommandType = CommandType.StoredProcedure
                    cmd.Parameters.Add(New SqlParameter("@Username", Me.UserName.Text.Trim))
                    cmd.Connection = con
                    con.Open()
                    ...
                    If dt.Rows.Count > 0 Then
                        Dim _str_accountname, _str_passwords, _str_salt As String
                        Try
                            _str_accountname = dt.Rows(0).Item("Username").ToString.Trim
                        Catch ex As Exception
                            _str_accountname = ""
                        End Try
                       System.Web.HttpContext.Current.Session("fff") = _str_accountname
                        Try
                            _str_passwords = dt.Rows(0).Item("Password").ToString.Trim
                        Catch ex As Exception
                            _str_passwords = ""
                        End Try

                        Try
                            _str_salt = dt.Rows(0).Item("Salt").ToString.Trim
                        Catch ex As Exception
                            _str_salt = ""
                        End Try
                        Dim _obj_encrypt As New SecurityClass
                        Dim str_text_pass As String = ""
                        'encrypt pass from textbox with salt from database
                        str_text_pass = _obj_encrypt.Decrypt(_str_passwords, _str_salt)
                        Dim k As String = Me.Password.Text
                        If k.Equals(str_text_pass) = True Then
                            Me.Password.Text = _str_passwords
							'Session("unamexxxx")=
        ...
        Public Sub Login(ByVal bRedirectOnSuccess As Boolean)
            ...
            Using con As New SqlConnection(strConnString)
                Using cmd As New SqlCommand()
                    ...
                    If dt.Rows.Count > 0 Then
                        Dim _str_accountname_encrypt, _str_passwords_encrypt, _str_salt_encrypt As String
                        Dim _str_accountname, _str_passwords, _str_salt As String
                        Try
                            _str_accountname_encrypt = dt.Rows(0).Item("Username").ToString.Trim
                        Catch ex As Exception
                            _str_accountname_encrypt = ""
                        End Try

                        Try
                            _str_passwords_encrypt = dt.Rows(0).Item("Password").ToString.Trim
                        Catch ex As Exception
                            _str_passwords_encrypt = ""
                        End Try

                        Try
                            _str_salt_encrypt = dt.Rows(0).Item("Salt").ToString.Trim
                        Catch ex As Exception
                            _str_salt_encrypt = ""
                        End Try
                        ...
                        'encrypt pass from textbox with salt from database
                        str_text_pass = _obj_encrypt.Decrypt(_str_passwords, _str_salt)
                        Dim k As String = Me.Password.Text.ToString.Trim
 ...
 '' Note, in case you need to return output value from the Stored procedure you might use the following code:
  ...
  '' parameterList(2) = thirdParameter
	...
	'' Step 3: Connect to the stored procedure using following line if 
            ...
            Dim CMS_CONNECTIONSTRING As String = _CMS.CMS_CONNECTIONSTRING
            myStoredProcedure = New BaseClasses.Data.StoredProcedure(CMS_CONNECTIONSTRING, "proc_admins_check_account", Nothing)
...
#Region "Code Customization"
	...
	''' </summary>
        Private Sub LoadData_CUSTOMIZE()
            ds = New DataSet

            ds = Me.CallCustomStoredProcedureForGridView()

            dt = New DataTable

            dt = ds.Tables(0)

            If dt IsNot Nothing = True Then

                If dt.Rows.Count > 0 Then

                    ' ' Me.Page.Response.Write(dt.Rows.Count.ToString + "</br>")
...
#Region " Protected Properties "
      ...
      Public ReadOnly Property ERR_INVALID_LOGIN_INFO() As String
        Get
          Return DirectCast(Me.Page, BaseApplicationPage).GetResourceValue("Err:InvalidLoginInfo", "CMStTrafficSafety")
        End Get
      ...
      Public ReadOnly Property ERR_INTERNAL_ERROR() As String
          Get
              Return DirectCast(Me.Page, BaseApplicationPage).GetResourceValue("Err:InternalErrorLogin", "CMStTrafficSafety")
          End Get
      ...
      Public ReadOnly Property LOGIN_MSG() As String
          Get
              Return DirectCast(Me.Page, BaseApplicationPage).GetResourceValue("Txt:LoginMsg", "CMStTrafficSafety")
          End Get
      ...
      Public ReadOnly Property LOGIN_MSG_SESSION_INVALID() As String
          Get
              Return DirectCast(Me.Page, BaseApplicationPage).GetResourceValue("Txt:LoginMsgSessionInvalid", "CMStTrafficSafety")
          End Get
